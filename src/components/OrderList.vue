<template>
  <div id="order-list">
    ...
    <div class="order-list-title">
      <h1 class="order-list-title-name">Your Orders</h1>
      <div class="order-list-title-filter">
          <!--    -1:已失效, 0:未支付, 1: 已完成, 2:全部 -->
        <el-button :class="orderStatus == 2 ? 'el-button--primary' : 'el-button--text'"
                   size="mini" round @click="changeOrderStatus(2)">All</el-button>
        <el-button :class="orderStatus == 0 ? 'el-button--primary' : 'el-button--text'"
                   size="mini" round @click="changeOrderStatus(0)">Cart</el-button>
        <el-button :class="orderStatus == 1 ? 'el-button--primary' : 'el-button--text'"
                   size="mini" round @click="changeOrderStatus(1)">Orders</el-button>
        <el-button :class="orderStatus == -1 ? 'el-button--primary' : 'el-button--text'"
                   size="mini" round @click="changeOrderStatus(-1)">Cancelled</el-button>
      </div>
      <div class="order-list-search" >
        <el-input placeholder="Search for anything" prefix-icon="el-icon-search" ></el-input>
      </div>
    </div>
        ...
        <div class="order-info">
          ...
          <div class="order-pay-info">
            <div v-if="item.payStatus == 1">
              <span>Paid</span><br/>
              <!-- <span>支付方式</span> -->
              <span v-if="item.payment == 0 ">Alipay</span>
              <span v-else-if="item.payment == 1">wechatpay</span>
              <span v-else>other payment</span>
            </div>
            <div v-else-if="item.payStatus == 0"><span>Unpaid</span></div>
            <div v-else><span>Failed</span></div>
            <!-- <div v-else> -->
